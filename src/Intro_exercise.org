...
And last, two multi-line variable definitions. The first contains
emacs configuration that I use to customize org-mode's export
functions, saved to a file =emacs-export-conf.ef=, and the second is
the small script that extracts the dependency information from the Org
source files.
